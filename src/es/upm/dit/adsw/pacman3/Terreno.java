...
//        System.out.println("Test conectividad: " + testConectividad());
    ...
    public synchronized Casilla move(Movil movil, Direccion direccion) {
    	 ...
    	 while(movil.puedoMoverme(direccion) == 2){
    	 notifyAll();
    	 return origen;
    	 }
    ...
    public  synchronized void quitaLlavesTrampas(){
		
			for (int i =0; i< getN(); i++){
				for (int j=0; j < getN(); j++){
					
					if (getCasilla(i, j).isLlave()){
						getCasilla(i, j).setLlave(false);
						 }
				    if (getCasilla(i, j).isTrampa()){
						getCasilla(i, j).setTrampa(false);
					     }
				}
			}
		}
    
    public synchronized Paredes getParedes(){
     return paredes;
    }
    }
